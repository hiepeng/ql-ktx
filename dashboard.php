...
<head>
	...
	<title>Trình theo dõi - Bảng điều khiển</title>
	...
	<div class="col-sm-9 col-sm-offset-3 col-lg-10 col-lg-offset-2 main">
		<div class="row">
			<ol class="breadcrumb">
				...
				<li class="active">Báo cáo tiền phòng</li>
			</ol>
		</div><!--/.row-->
		
		<div class="row">
			<div class="col-lg-12">
				<h1 class="page-header">Báo cáo tiền phòng</h1>
			</div>
		</div><!--/.row-->
...
$tdate=date('Y-m-d');
$query=mysqli_query($con,"select sum(GiaThue)  as todaysexpense from thuephong where (NgayThue)='$tdate' && (UserId='$userid');");
...
$query1=mysqli_query($con,"select sum(GiaThue)  as yesterdayexpense from thuephong where (NgayThue)='$ydate' && (UserId='$userid');");
$result1=mysqli_fetch_array($query1);
$sum_yesterday_expense=$result1['yesterdayexpense'];
 ?> 
					<div class="panel-body easypiechart-panel">
						<h4>Tổng tiền phòng thu được ngày hôm qua</h4>
...
$crrntdte=date("Y-m-d");
$query2=mysqli_query($con,"select sum(GiaThue)  as weeklyexpense from thuephong where ((NgayThue) between '$pastdate' and '$crrntdte') && (UserId='$userid');");
$result2=mysqli_fetch_array($query2);
$sum_weekly_expense=$result2['weeklyexpense'];
 ?>
					<div class="panel-body easypiechart-panel">
						<h4>Tổng tiền phòng thu được tuần qua</h4>
...
$crrntdte=date("Y-m-d");
$query3=mysqli_query($con,"select sum(GiaThue)  as monthlyexpense from thuephong where ((NgayThue) between '$monthdate' and '$crrntdte') && (UserId='$userid');");
$result3=mysqli_fetch_array($query3);
$sum_monthly_expense=$result3['monthlyexpense'];
 ?>
					<div class="panel-body easypiechart-panel">
						<h4>Tổng tiền phòng thu được 30 ngày gần nhất</h4>
...
$userid=$_SESSION['detsuid'];
 $cyear= date("Y");
$query4=mysqli_query($con,"select sum(GiaThue)  as yearlyexpense from thuephong where (year(NgayThue)='$cyear') && (UserId='$userid');");
$result4=mysqli_fetch_array($query4);
$sum_yearly_expense=$result4['yearlyexpense'];
 ?>
					<div class="panel-body easypiechart-panel">
						<h4>Tổng tiền phòng thu được năm nay</h4>
...
//Yearly Expense
$userid=$_SESSION['detsuid'];
$query5=mysqli_query($con,"select sum(GiaThue)  as totalexpense from thuephong where UserId='$userid';");
$result5=mysqli_fetch_array($query5);
$sum_total_expense=$result5['totalexpense'];
 ?>
					<div class="panel-body easypiechart-panel">
						<h4>Tổng thu tiền phòng</h4>
